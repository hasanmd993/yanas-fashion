...
@section('content')
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center gap-2 admin-actions-wrap">
            ...
            <a href="{{ route('admin.orders.export_pdf', ['status' => request('status', 'all'), 'search' => request('search')]) }}"
                target="_blank"
                class="btn inline-flex items-center gap-2 rounded-lg bg-[#e11d48] px-3.5 py-2 text-xs font-bold text-white shadow-sm hover:bg-[#be123c] transition-all"
                title="View filtered orders report PDF in new tab">
                <i class="fa-solid fa-file-pdf"></i> Export PDF
            </a>
            <!-- <a href="{{ route('admin.orders.export_courier', ['status' => request('status', 'pending')]) }}"
                        class="btn inline-flex items-center gap-2 rounded-lg bg-secondary px-3.5 py-2 text-xs font-bold text-white shadow-sm hover:bg-secondary-hover transition-all"
                        title="Bulk Courier Format for Steadfast / Pathao">
