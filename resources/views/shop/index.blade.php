    ...
    <div style="background: var(--bg-light); border-bottom: 1px solid var(--line); padding: 24px 0;">
        <div class="container">
            ...
            <h1 style="font-size: 2rem;">
                {{ $currentCategory ? $currentCategory->name : 'সব কালেকশন (All Collections)' }}
            </h1>
            <p style="color:var(--text-muted); font-size:0.92rem;">
                {{ $currentCategory ? $currentCategory->description : 'বাংলার ঐতিহ্য ও আধুনিক আভিজাত্যে তৈরি আমাদের সকল এক্সক্লুসিভ পোশাক কালেকশন।' }}
            </p>
        </div>
    </div>
        ...
        <div class="container">
            ...
            <div style="display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: 16px; margin-bottom: 24px; padding-bottom: 16px; border-bottom: 1px solid var(--line);">
                ...
                <form action="{{ route('shop.index') }}" method="GET" style="display: flex; align-items: center; gap: 8px;">
                    ...
                    @endif
                    <label for="sort-select" style="font-size: 0.85rem; font-weight: 600; color: var(--text-muted);">সর্ট করুন:</label>
                    <select name="sort" id="sort-select" onchange="this.form.submit()" 
                        ...
                        <option value="latest" {{ request('sort') == 'latest' ? 'selected' : '' }}>নতুন আগমন (Latest)</option>
                        <option value="price_low" {{ request('sort') == 'price_low' ? 'selected' : '' }}>মূল্য: কম থেকে বেশি (Price Low to High)</option>
                        <option value="price_high" {{ request('sort') == 'price_high' ? 'selected' : '' }}>মূল্য: বেশি থেকে কম (Price High to Low)</option>
                        <option value="popular" {{ request('sort') == 'popular' ? 'selected' : '' }}>সর্বাধিক জনপ্রিয় (Popular)</option>
                    </select>
                </form>
            </div>
            ...
            @if($products->isEmpty())
                <div style="text-align: center; padding: 64px 16px; background: var(--white); border-radius: var(--radius-md); border: 1px solid var(--line);">
                    ...
                    <h3 style="margin-bottom: 8px;">কোন পণ্য পাওয়া যায়নি</h3>
                    <p style="color: var(--text-muted); margin-bottom: 16px;">অনুগ্রহ করে অন্য ক্যাটাগরি বা ফিল্টার ব্যবহার করে দেখুন।</p>
                    <a href="{{ route('shop.index') }}" class="btn btn-primary btn-sm">সব পণ্য দেখুন</a>
                </div>
            @else
