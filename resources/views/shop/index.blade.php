...
@section('content')
    ...
    <div style="background: var(--bg-light); border-bottom: 1px solid var(--line); padding: 24px 0;">
        <div class="container">
            <nav aria-label="breadcrumb" style="display:flex; flex-wrap:wrap; align-items:center; gap:8px; font-size:0.82rem; color:var(--text-muted); margin-bottom:10px;">
                ...
                @if($currentCategory)
                    ...
                    @foreach($crumbs as $index => $crumb)
                        ...
                        @else
                            <a href="{{ route('shop.index', ['category' => $crumb->slug]) }}" style="color:var(--text-muted); text-decoration:none;">
                                {{ $crumb->name }}
                            </a>
                        @endif
                    @endforeach
    ...
    <section style="padding: 32px 0 64px;">
        <div class="container">
            ...
            <div style="display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: 16px; margin-bottom: 16px; padding-bottom: 16px; border-bottom: 1px solid var(--line);">
            ...
            @if($activeParentCategory && $activeParentCategory->activeChildren->count() > 0)
                <div style="background: var(--bg-light); border: 1px solid var(--line); border-radius: var(--radius-md); padding: 14px 18px; margin-bottom: 28px;">
                    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:10px;">
                        <span style="font-size:0.75rem; text-transform:uppercase; font-weight:800; letter-spacing:0.06em; color:var(--text-muted);">
            ...
            @if($products->isEmpty())
                <div style="text-align: center; padding: 64px 16px; background: var(--white); border-radius: var(--radius-md); border: 1px solid var(--line);">
                    ...
                    <p style="color: var(--text-muted); margin-bottom: 16px;">Try selecting a parent category or browse all collections.</p>
                    ...
                    @foreach($products as $product)
                        <div class="product-card">
                            ...
                            <div class="product-info">
                                ...
                                <div class="product-card-btns" style="margin-top: 12px; display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">
                                    <button type="button" class="btn btn-outline btn-sm quick-add-cart" 
                                            data-id="{{ $product->id }}" 
